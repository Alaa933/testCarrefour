...
class TopChiffreAffaire {
  ...
  def chiffreAffaireParMagasin(inputTransactionsFiles:ListBuffer[Transaction], path:String): Unit ={
    var chiffreAffaireParMagasin =0
    var list7DerniersJourS: ListBuffer[ChiffreDaffaire]=ListBuffer[ChiffreDaffaire]()
    val folder = new File(path + "/input/magasins")
    if (folder.exists && folder.isDirectory)
      folder.listFiles
        .toList
        .foreach(file => {
          var sum =0
          var listParMagasinParDate = inputTransactionsFiles
            .filter(_.datetime.slice(0,8).equals(file.getName.split("_")(2).slice(0,8)))
            .filter(_.magasin.equals(file.getName.split("_")(1).substring(5)))
          Source.fromFile(file.getAbsolutePath)
            .getLines()
            .foreach(line => {
                 listParMagasinParDate.filter(_.produit ==line.split("\\|")(0).toInt)
                   .groupBy(_.produit)
                   .foreach(x=>{
                  x._2.foreach(x=>{
                  ...
                  list7DerniersJourS
                    .append(ChiffreDaffaire(file.getName.split("_")(2).slice(0,8),file.getName.split("_")(1)
                    .substring(5),x._1, sum))
                })
                })
        })
