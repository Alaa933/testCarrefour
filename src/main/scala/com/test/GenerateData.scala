...
class GenerateData(){
  ...
  def generateTransactions(year:String, month:String,firstDay:Int, lastDay:Int,listDesMagasins:ListBuffer[String]): Unit = {
    ...
    for(day <- firstDay to lastDay){
      ...
      val file = new File(String.format("/home/alaa/IdeaProjects/carrefour/target/classes/data/input/transactions/transactions_%s.data", date))
      val bw = new BufferedWriter(new FileWriter(file))
      for (i <- 1 to nbr_transactions) {
        bw.write(i + "|" + date+'T'+r.nextInt(2)+r.nextInt(2)+r.nextInt(5)+r.nextInt(9)+r.nextInt(5)+r.nextInt(9) + "+0100" + "|" + listDesMagasins(r.nextInt(listDesMagasins.length)) + "|" + (r.nextInt(nbr_produit)+1) + "|" + r.nextInt(qté_max) + "\n")
      }
      bw.close()
    }
  }
  ...
  def generateReferences(year:String, month:String,firstDay:Int, lastDay:Int, listDesMagasins:ListBuffer[String]): Unit = {
    println(listDesMagasins.size)
    for(day <- firstDay to lastDay){
      var date:String = year+ month +day.toString
      for(magasin <- listDesMagasins){
        val file = new File(String.format("/home/alaa/IdeaProjects/carrefour/target/classes/data/input/magasins/reference_prod-%s_%s.data", magasin, date))
        val bw = new BufferedWriter(new FileWriter(file))
